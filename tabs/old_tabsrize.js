...
const isFieldValid = (fieldView, fieldType) => {
    ...
    if (fieldType === 'listbox') {
        const isEmpty = key === "0" || key === 0 || value === null || value === undefined || value === '' || value === 'Не выбрано' || value === '—' || value === '–' || value === '-';
    ...
    // !!! НОВОЕ ИЗМЕНЕНИЕ: Отдельная проверка для numericinput !!!
    else if (fieldType === 'numericinput') {
        const isEmpty = value === null || value === undefined || value === '' || value === '—' || value === '–' || value === '-';
         if (fieldView.model.getSpecialErrors) {
            try {
                const errors = fieldView.model.getSpecialErrors();
                if (errors && typeof errors === 'object' && errors.errorCode) {
    ...
    // Проверка на пустоту для всех типов (textbox и т.д.)
    // !!! ИСПРАВЛЕНИЕ: Добавлена проверка на прочерки для ВСЕХ ТИПОВ !!!
...
const isTabEmpty = (tabIndex) => {
    ...
    // Если таб явно скрыт
    if (tabData.hasOwnProperty('visible') && tabData.visible === false) {
        return true;
    }
    ...
    allBlocks.forEach(blockId => {
        try {
            ...
            if (blockView && blockView.tableModel) {
                // Проверяем свойство visible для блока
...
const updateTabsValidation = () => {
    tabsData.forEach((item, i) => {
        ...
        // Проверяем, пустой ли таб или все его блоки скрыты
        if (isTabEmpty(i)) {
